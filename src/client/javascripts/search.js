export default {
  ...
  setupSearch () {
    this.searchInput = document.getElementById('search-input') || document.getElementById('results-search-input')
    this.domSuggestions = document.getElementById('suggestions')
  },

  async getSearchSuggestions (searchString) {
    return new Promise((resolve, reject) => {
      this.makeSearchRequest(searchString, (err, success) => {
        if (err) {
          reject(err)
          return
        }
  ...
  setActive () {
    ...
    if ([this.loadingText, this.noResultsText].includes(this.domSuggestions.children[this.focusIndex].textContent)) {
      return
    }
